...
export function ParticleSystem({ colors }: ParticleSystemProps = {}) {
  ...
  useEffect(() => {
    // Verificar si debemos mostrar el efecto (rendimiento)
    const deviceInfo = localStorage.getItem("deviceInfo");
    if (deviceInfo) {
      const { prefersReducedMotion } = JSON.parse(deviceInfo);
      const isLowPower = document.body.classList.contains("low-power-mode");

      if (isLowPower || prefersReducedMotion) {
        setShouldRender(false);
        console.log("🚫 Efecto LiquidEther desactivado para mejor rendimiento");
      }
    }
  }, []);
...
// Implementación interna de LiquidEther
