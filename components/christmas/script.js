/* Integra el final de animación con la web embebida: envía `christmas:done` al padre para ocultar el overlay. */
(function () {
  function o(a) {
    try {
      parent && parent.postMessage && parent.postMessage(a, "*");
    } catch (b) {}
  }
  window.addEventListener(
    ...
    function (a) {
      o({
        type: "christmas:error",
        message: a && a.message ? a.message : "error",
      });
    },
    !0
  );
  window.addEventListener("unhandledrejection", function (a) {
    var b = a && a.reason ? a.reason : "unhandledrejection";
    o({ type: "christmas:error", message: "" + b });
  });
  window.addEventListener("beforeunload", function () {
    o({ type: "christmas:done" });
  });
  function r(a) {
  ...
  c.eventCallback("onComplete", function () {
    try {
      parent &&
        parent.postMessage &&
        parent.postMessage({ type: "christmas:done" }, "*");
    } catch (a) {}
  });
